...
- Stickiness mechanism for path reuse
...
class NCRPTreeParticle:
    """Stores per-particle CRP counts at each level."""
    ...
    def __init__(self, alpha, max_depth, max_children):
        ...
        Args:
            ...
            max_children: Maximum branches per level
        """
        self.alpha = float(alpha)
        self.max_depth = int(max_depth)
        self.max_children = max_children
        # level_counts[L] = list of counts for each branch at level L
...
class MHLCMCRP:
    ...
    def __init__(
        ...
        Args:
            ...
            depth_decay: Rate of decay for concentration with depth
            max_children: Maximum number of children per node
            stickiness: Bonus weight for reusing previous paths
        """
        self.alpha = float(alpha)
        self.max_depth = int(max_depth)
        ...
        for m in range(self.nParticles):
            ...
            for L in range(self.max_depth):
                ...
                if L > 0:
                    ...
                    #stop_prob = self.alpha / (1.0 + self.alpha)
                    if self.rng.rand() < stop_prob:
                        break

                # Depth-decayed concentration option 2: alpha_L = alpha * exp(-depth_decay * L)
                ...
                if K == 0:
                    # No existing branches: must create first one
                    probs = np.array([1.0])
                    choice = 0
                    ...
                    if K < self.max_children:
                        ...
                        # At capacity: choose existing only
                    ...
                    if (prev_path is not None and 
                        ...
                        for existing in range(K):
                            ...
                            if candidate_gid == prev_gid:
                                # Multiplicative bonus to encourage reuse
...
def full_hier_inference_loop(
    ...
    for t in range(nTrials):
        ...
        # Make predictions using CURRENT parameters (before curr data)
        ...
        for l in range(nParticles):
            ...
            if has_fb:
                # Full feedback: update all observed features
                # Convert to int to ensure we're adding counts, not booleans
                nFC[:, k, l] += obs.astype(int) * obs_mask_vec.astype(int)
                bFC[:, k, l] += ((1 - obs).astype(int) * 
                                 obs_mask_vec.astype(int))
            else:
