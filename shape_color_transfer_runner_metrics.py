...
import argparse, importlib, json, inspect
...
def majority_path_for_trial(paths: np.ndarray, t: int) -> np.ndarray:
    """Return majority global/dense node ID at each depth for trial t."""
    max_depth = paths.shape[2]
    out = -np.ones(max_depth, dtype=int)
    for L in range(max_depth):
        out[L] = majority_id(paths[:, t, L])
    return out


def compute_node_shape_reward_table(paths: np.ndarray, shape: np.ndarray, y: np.ndarray, mask: np.ndarray) -> pd.DataFrame:
    """Summarize node purity and reward rate by level using majority node assignments.

    This is post-hoc diagnostic only. It asks whether a node, as used in the
    final paths, is shape-pure and reward-informative.
    """
    rows = []
    for L in range(paths.shape[2]):
        assign = np.array([majority_id(paths[:, t, L]) for t in range(paths.shape[1])], dtype=int)
        valid_nodes = sorted(np.unique(assign[mask & (assign >= 0)]))
        for node in valid_nodes:
            idx = mask & (assign == node)
            if not np.any(idx):
                continue
            n = int(idx.sum())
            p_shape0 = float(np.mean(shape[idx] == 0))
            p_reward = float(np.mean(y[idx]))
            purity = float(max(p_shape0, 1.0 - p_shape0))
            rows.append({
                "level": int(L),
                "node": int(node),
                "n_train_trials": n,
                "p_shape0": p_shape0,
                "shape_purity": purity,
                "reward_rate": p_reward,
                "informativeness": abs(p_reward - 0.5) * n,
            })
    return pd.DataFrame(rows)


def lookup_node_summary(node_table: pd.DataFrame, level: int, node: int) -> Dict[str, float]:
    if node_table is None or len(node_table) == 0 or level < 0 or node < 0:
        return {
            "node_train_n": np.nan,
            "node_train_p_shape0": np.nan,
            "node_train_shape_purity": np.nan,
            "node_train_reward_rate": np.nan,
            "node_train_informativeness": np.nan,
        }
    sub = node_table[(node_table["level"] == level) & (node_table["node"] == node)]
    if len(sub) == 0:
        return {
            "node_train_n": 0,
            "node_train_p_shape0": np.nan,
            "node_train_shape_purity": np.nan,
            "node_train_reward_rate": np.nan,
            "node_train_informativeness": 0.0,
        }
    row = sub.iloc[0]
    return {
        "node_train_n": float(row["n_train_trials"]),
        "node_train_p_shape0": float(row["p_shape0"]),
        "node_train_shape_purity": float(row["shape_purity"]),
        "node_train_reward_rate": float(row["reward_rate"]),
        "node_train_informativeness": float(row["informativeness"]),
    }


def best_posthoc_path_node_for_trial(path: np.ndarray, node_table: pd.DataFrame) -> Dict[str, float]:
    """Diagnostic oracle over nodes on observed path, based only on train node summaries.

    This is not used for model prediction. It tells us whether the *path contains*
    a reward-informative/shape-pure node that a better readout could exploit.
    """
    best = {
        "best_path_diag_level": -1,
        "best_path_diag_node": -1,
        "best_path_diag_reward_rate": np.nan,
        "best_path_diag_n": 0.0,
        "best_path_diag_score": 0.0,
    }
    if node_table is None or len(node_table) == 0:
        return best
    for L, node in enumerate(path):
        if node < 0:
            continue
        sub = node_table[(node_table["level"] == L) & (node_table["node"] == node)]
        if len(sub) == 0:
            continue
        row = sub.iloc[0]
        score = float(row["informativeness"])
        if score > best["best_path_diag_score"]:
            best = {
                "best_path_diag_level": int(L),
                "best_path_diag_node": int(node),
                "best_path_diag_reward_rate": float(row["reward_rate"]),
                "best_path_diag_n": float(row["n_train_trials"]),
                "best_path_diag_score": score,
            }
    return best
...
def run_one_seed(seed: int, cfg: ShapeColorConfig, hier_module: str, n_particles: int,
    ...
    # Post-hoc diagnostic summaries of which nodes actually contain shape/reward
    # information in the training phase. These do not affect model predictions.
    node_table = compute_node_shape_reward_table(paths_hier, shape, y, train_mask)
    if len(node_table):
        top_nodes = node_table.sort_values("informativeness", ascending=False).head(10)
    else:
    ...
    result = {
        ...
        "flat_train_shape0_mean_prob": float(np.mean(r_flat[train_mask & (shape == 0)])) if np.any(train_mask & (shape == 0)) else np.nan,
        "flat_train_shape1_mean_prob": float(np.mean(r_flat[train_mask & (shape == 1)])) if np.any(train_mask & (shape == 1)) else np.nan,
        "hier_train_shape0_mean_prob": float(np.mean(r_hier[train_mask & (shape == 0)])) if np.any(train_mask & (shape == 0)) else np.nan,
        "hier_train_shape1_mean_prob": float(np.mean(r_hier[train_mask & (shape == 1)])) if np.any(train_mask & (shape == 1)) else np.nan,
        "flat_heldout_shape0_mean_prob": float(np.mean(r_flat[first_heldout_mask & (shape == 0)])) if np.any(first_heldout_mask & (shape == 0)) else np.nan,
        "flat_heldout_shape1_mean_prob": float(np.mean(r_flat[first_heldout_mask & (shape == 1)])) if np.any(first_heldout_mask & (shape == 1)) else np.nan,
        "hier_heldout_shape0_mean_prob": float(np.mean(r_hier[first_heldout_mask & (shape == 0)])) if np.any(first_heldout_mask & (shape == 0)) else np.nan,
        "hier_heldout_shape1_mean_prob": float(np.mean(r_hier[first_heldout_mask & (shape == 1)])) if np.any(first_heldout_mask & (shape == 1)) else np.nan,
        ...
        # Does the diagnostic best shape level also carry reward information?
        "hier_best_level_mean_reward_rate": float(node_table[node_table["level"] == best_L]["reward_rate"].mean()) if best_L >= 0 and len(node_table) and np.any(node_table["level"] == best_L) else np.nan,
        "hier_best_level_max_informativeness": float(node_table[node_table["level"] == best_L]["informativeness"].max()) if best_L >= 0 and len(node_table) and np.any(node_table["level"] == best_L) else np.nan,
        "hier_top_reward_node_level": int(top_nodes.iloc[0]["level"]) if len(top_nodes) else -1,
        "hier_top_reward_node": int(top_nodes.iloc[0]["node"]) if len(top_nodes) else -1,
        "hier_top_reward_node_rate": float(top_nodes.iloc[0]["reward_rate"]) if len(top_nodes) else np.nan,
        "hier_top_reward_node_p_shape0": float(top_nodes.iloc[0]["p_shape0"]) if len(top_nodes) else np.nan,
        "hier_top_reward_node_n": float(top_nodes.iloc[0]["n_train_trials"]) if len(top_nodes) else 0.0,
    }
    ...
    result.update({
        # Continuous probabilistic metrics; less threshold-sensitive than binary accuracy.
    ...
    for t in np.where(first_heldout_mask)[0]:
        rows.append({
            "seed": seed, "trial": int(t), "shape": int(shape[t]), "color": int(color[t]),
            "reward": int(y[t]), "flat_prob": float(r_flat[t]), "hier_prob": float(r_hier[t]),
            "flat_correct": int((r_flat[t] > 0.5) == y[t]),
            "hier_correct": int((r_hier[t] > 0.5) == y[t]),
            "flat_cluster": int(flat_assign[t]), "hier_best_shape_level": int(best_L),
            "hier_best_node": int(hier_best_assign[t]),
        })
        # Add diagnostics for the majority path and node reward information.
        path_t = majority_path_for_trial(paths_hier, t)
        row = rows[-1]
        row["hier_majority_path"] = "|".join(str(int(x)) for x in path_t if x >= 0)
        row["hier_path_depth"] = int(np.sum(path_t >= 0))
        # Node summary at the level selected by shape-diagnostic alignment.
        row.update(lookup_node_summary(node_table, int(best_L), int(hier_best_assign[t]) if best_L >= 0 else -1))
        # Best reward-informative diagnostic node anywhere on this test path.
        row.update(best_posthoc_path_node_for_trial(path_t, node_table))
        # If model returned actual online readout debug, attach it.
        if 'tree_debug' in locals() and tree_debug is not None:
            try:
                row["online_readout_level"] = int(tree_debug["chosen_depth"][t])
                row["online_readout_node"] = int(tree_debug["chosen_node"][t])
                row["online_readout_prob"] = float(tree_debug["chosen_prob"][t])
                row["online_readout_evidence"] = float(tree_debug["chosen_evidence"][t])
                row["online_readout_score"] = float(tree_debug["chosen_score"][t])
            except Exception:
                pass
...
def main():
    ...
    parser.add_argument("--train_reps", type=int, default=10)
    parser.add_argument("--test_reps", type=int, default=1)
    parser.add_argument("--shape_copies", type=int, default=2)
    parser.add_argument("--color_copies", type=int, default=4)
    ...
    for seed in args.seeds:
        ...
        results.append(res); trial_dfs.append(trial_df)
        ...
        if args.debug:
            ...
            if 'hier_top_reward_node_level' in res:
                print("  DEBUG top reward-informative training node:")
                print(f"    level={res['hier_top_reward_node_level']}, node={res['hier_top_reward_node']}, "
                      f"reward_rate={res['hier_top_reward_node_rate']:.3f}, "
                      f"p_shape0={res['hier_top_reward_node_p_shape0']:.3f}, "
                      f"n={res['hier_top_reward_node_n']:.0f}")
            ...
            if len(trial_df):
                ...
                extra_dbg = [
                    'hier_majority_path', 'hier_path_depth',
                    'node_train_n', 'node_train_reward_rate', 'node_train_shape_purity',
                    'best_path_diag_level', 'best_path_diag_node',
                    'best_path_diag_reward_rate', 'best_path_diag_n', 'best_path_diag_score',
                    'online_readout_level', 'online_readout_node', 'online_readout_prob',
                    'online_readout_evidence', 'online_readout_score'
                ]
                cols_dbg = cols_dbg + extra_dbg
    ...
    cols = [
        ...
        "hier_top_reward_node_level", "hier_top_reward_node_rate", "hier_top_reward_node_p_shape0",
    ...
    print("\nMean heldout shape contrast (shape0 - shape1; higher is better):")
    ...
    trials_path = out_prefix.with_name(out_prefix.name + "_first_heldout_trials.csv")
    ...
    print(f"Saved first-heldout trials to: {trials_path}")
