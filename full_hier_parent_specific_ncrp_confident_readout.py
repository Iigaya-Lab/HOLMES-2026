    ...
    def __init__(
        ...
        Args:
            ...
            depth_decay: Rate of decay for concentration with depth. If None,
                uses alpha * depth_scale, implementing the relaxed
                bounded-rationality coupling
                alpha_l = alpha * exp(-depth_scale * alpha * level).
                At depth_scale=1.0 this recovers the original paper coupling.
                At depth_scale=0.0 concentration is constant across levels.
            max_children: Maximum number of children per node
            stickiness: Base persistence strength. In the HOLMES paper logic
                this is tied to omega. The level-specific value is
                kappa_l = stickiness * exp(-stick_scale * stickiness_decay * level).
            stickiness_decay: Depth decay for kappa_l. If None, uses stickiness
                * stick_scale, giving the relaxed schedule
                kappa_l = omega * exp(-stick_scale * omega * level).
                At stick_scale=1.0 this recovers the original paper coupling.
                At stick_scale=0.0 stickiness is constant across levels.
            ...
            depth_scale: Scalar in [0, 1] that relaxes the coupling between
                alpha and the depth-decay rate.  1.0 = original paper assumption;
                0.0 = no decay.  Preserves the structural claim that alpha
                controls capacity while allowing deeper trees at high alpha.
            stick_scale: Scalar in [0, 1] that relaxes the coupling between
                omega and the stickiness-decay rate.  1.0 = original paper
                assumption; 0.0 = constant stickiness at all depths.
        """
        self.alpha = float(alpha)
        self.max_depth = int(max_depth)
        self.nParticles = nParticles
        self.nMaxCauses = nMaxCauses
        self.depth_scale = float(np.clip(depth_scale, 0.0, 1.0))
        self.stick_scale = float(np.clip(stick_scale, 0.0, 1.0))

        # Relaxed coupling: alpha_l = alpha * exp(-depth_scale * alpha * level).
        # depth_decay may still be passed explicitly to override entirely.
        self.depth_decay = float(
            alpha * self.depth_scale if depth_decay is None else depth_decay
        )
        self.max_children = max_children

        # Relaxed coupling: kappa_l = omega * exp(-stick_scale * omega * level).
        self.stickiness = float(stickiness)
        self.stickiness_decay = float(
            stickiness * self.stick_scale if stickiness_decay is None else stickiness_decay
        )
        ...
        for m in range(self.nParticles):
            ...
            for L in range(self.max_depth):
                ...
                if K == 0:
                    ...
                    #   P(branch k) prop. to  n_k + kappa_l * 1[k = previous branch]
                    # and P(new) prop. to alpha_l.
...
def full_hier_inference_loop(
    ...
    Args:
        ...
        depth_scale: Scalar in [0, 1] relaxing the alpha-depth-decay coupling.
            alpha_l = alpha * exp(-depth_scale * alpha * level).
            1.0 = original paper assumption (exact coupling).
            0.0 = no depth decay (constant concentration at all levels).
        stick_scale: Scalar in [0, 1] relaxing the omega-stickiness-decay coupling.
            kappa_l = omega * exp(-stick_scale * omega * level).
            1.0 = original paper assumption (exact coupling).
            0.0 = no stickiness decay (constant persistence at all levels).
    ...
    if feedback_mask is None:
        ...
        if feedback_mask.ndim == 1:
            ...
            raise ValueError("feedback_mask must be None, (T,) or (nFeatures, T). try harder next time. feedback_mask.ndim = {feedback_mask.ndim}")
    ...
    prior = MHLCMCRP(
        ...
        depth_decay=None,           # use relaxed coupling via depth_scale
    ...
    for t in range(nTrials):
        ...
        for l in range(nParticles):
            ...
            if has_fb:
                ...
                # learn from the reward on this trial for next
                ...
                # Also update every valid node on the path to pool ev. across leaves
                if use_tree_prediction:
        ...
        if not np.all(np.isfinite(wt)) or not np.all(wt >= 0):
            ...
            # Reset to uniform if something went wrong and scream
            wt = np.ones(nParticles) / nParticles
            rt = np.ones(nParticles) / nParticles
